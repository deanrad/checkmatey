...
// and make sure to return Observables of immutables
 const Consequences = {
    'Game.requestMove': (action, action$) => {
        ...
        let from = action.getIn(['payload', 'from'])
        let promotionOption = action.getIn(['payload', 'promotionOption'])
        ...
        if (pieceType === 'P' && reachedOpposingHome && !promotionOption) {
            return Observable.of(fromJS(Actions.Game.showPromotionOption({ from, to, color })))
        }
